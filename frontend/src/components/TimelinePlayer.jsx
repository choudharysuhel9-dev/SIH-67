...
export default function TimelinePlayer({
  ...
  return (
    <div className="bg-[#091524]/95 backdrop-blur-md border border-[#1C3652] rounded-xl px-3 sm:px-4 py-2 sm:py-2.5 shadow-2xl flex flex-col sm:flex-row items-center gap-2 sm:gap-4 w-full text-xs text-[#DCE8F0]">
      {/* Play / Step Controls & Speed Multiplier */}
      <div className="flex items-center justify-between w-full sm:w-auto gap-2">
        <div className="flex items-center gap-1">
          <button
            onClick={() => onChangeTimeStep(Math.max(0, currentTimeStep - 1))}
            className="p-1 sm:p-1.5 rounded-lg text-[#7C98B3] hover:text-white hover:bg-[#13283E] transition-colors"
          ...
          <button
            ...
            className={`p-1.5 sm:p-2 rounded-lg font-medium flex items-center justify-center transition-all ${
              isPlaying
        ...
        {/* Mobile current date tag */}
        <div className="sm:hidden flex items-center gap-1 text-[11px] font-mono">
          <span className="text-[#38BDF8] font-semibold">{activeDateInfo.day}</span>
          <span className="text-[#7C98B3]">({activeDateInfo.label})</span>
        </div>

        {/* Speed buttons */}
        <div className="flex items-center gap-0.5 bg-[#0A1828] border border-[#172F47] rounded-lg p-0.5 text-[9px] sm:text-[10px]">
          {[1, 5, 24].map((speed) => (
            <button
              key={speed}
      ...
      <div className="w-full flex-1 flex flex-col gap-1">
        <div className="hidden sm:flex justify-between items-baseline text-[11px]">
        ...
        {/* Step markers (Desktop only) */}
        <div className="hidden sm:flex justify-between text-[9px] font-mono text-[#557593]">

...
import { SlidersHorizontal, RotateCcw, BarChart2 } from "lucide-react";
...
export default function ColorbarEditor({
  ...
  return (
    <div className="bg-[#091524]/90 backdrop-blur-md border border-[#1C3652] rounded-xl p-3 shadow-xl w-64 max-w-[calc(100vw-2rem)] transition-all">
      {/* Header */}
      ...
      <div className="relative my-2">
        <div className={`h-3 rounded-md bg-gradient-to-r ${PALETTES[palette]?.gradient || PALETTES.Thermal.gradient} shadow-inner border border-white/10`} />
        {/* Step ticks */}
        <div className="flex justify-between text-[9px] font-mono text-[#6E879E] mt-1 px-0.5">
          <span>{min}</span>
      ...
      <div className="flex justify-between items-center gap-2 mt-2">
        <div className="flex items-center gap-1 bg-[#0E2033] border border-[#1A334E] rounded px-1.5 py-1">
          <span className="text-[10px] text-[#557593]">Min:</span>
          <input
            type="number"
            value={min}
            onChange={(e) => setMin(Number(e.target.value))}
            className="w-10 bg-transparent text-xs text-[#E2EDF8] font-mono focus:outline-none"
          />
        </div>
        ...
        <div className="flex items-center gap-1 bg-[#0E2033] border border-[#1A334E] rounded px-1.5 py-1">
          <span className="text-[10px] text-[#557593]">Max:</span>
          <input
            type="number"
            value={max}
            onChange={(e) => setMax(Number(e.target.value))}
            className="w-10 bg-transparent text-xs text-[#E2EDF8] font-mono text-right focus:outline-none"
          />
        </div>
      </div>
      ...
      {/* Expandable options */}
      {isExpanded && (
        <div className="mt-3 pt-3 border-t border-[#192F47] flex flex-col gap-2 animate-in fade-in slide-in-from-top-2 duration-150">
          <div>
            <span className="text-[10px] text-[#7C98B3] uppercase tracking-wider block mb-1">
              Colormap Palette
            </span>
            <select
              value={palette}
              onChange={(e) => handlePaletteSelect(e.target.value)}
              className="w-full bg-[#0E2033] text-xs text-[#DCE8F0] border border-[#1B324D] rounded px-2 py-1 focus:outline-none focus:border-[#38BDF8]"
            >
          ...
          <div className="flex items-center justify-between mt-1">
            <span className="text-[10px] text-[#7C98B3]">Scale Mapping:</span>
